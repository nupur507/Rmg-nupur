...
class UserController extends Controller
{
    ...
    public function submitProfile(Request $request)
    {
        $request->validate([
            ...
            'nominee_relation' => 'required|max:60',
            'nominee_dob' => 'required|max:60',
        ],[
        ...
        $in['nominee_dob'] =Carbon::parse($request->nominee_dob)->toDateString();
        $in['nominee_name'] = $request->nominee_name;
        $in['nominee_relation'] = $request->nominee_relation;
    ...
    public function updatekyc()
    {
        ...
        if(!$plan){
            $notify[] = ['error', 'You can not update kyc'];
            return back()->withNotify($notify);
        }
        $data['kyc'] = UserKyc::where('user_id', $user->id)->first();

        $data['page_title'] = "Update KYC";
    ...
    public function submitkyc(Request $request)
    {
        ...
        $request->validate([
            'adhaar_no' => 'required|string|max:50',
            'adhaar_file' => 'required|mimes:png,jpg,jpeg',
            'pancard' => "required|string|max:50",
            'pan_file' => "required|mimes:png,jpg,jpeg",
            'bank_file' => "required|mimes:png,jpg,jpeg",
        ...
        if ($request->hasFile('pan_file')) {
            ...
            $filename = time() . '_pan_' . $user->username . '.jpg';
            $location = 'assets/images/user/kyc/' . $filename;
            $in['pan_file'] = $filename;
        ...
        if ($request->hasFile('adhaar_file')) {
            $image = $request->file('adhaar_file');
            $filename = time() . '_adhaar_' . $user->username . '.jpg';
            $location = 'assets/images/user/kyc/' . $filename;
            $in['adhaar_file'] = $filename;
        ...
        if ($request->hasFile('bank_file')) {
            $image = $request->file('bank_file');
            $filename = time() . '_bank_' . $user->username . '.jpg';
            $location = 'assets/images/user/kyc/' . $filename;
            $in['bank_file'] = $filename;
        ...
        $kycexist = UserKyc::where('user_id', $id)->first();

        if($kycexist){
            $kycexist->pan_file =$in['pan_file'];
            $kycexist->adhaar_file =$in['adhaar_file'];
            $kycexist->bank_file =$in['bank_file'];
            $kycexist->adhaar_no =$request->adhaar_no;
            $kycexist->pancard =$request->pancard;
            $kycexist->is_approved =0;
            $kycexist->save();
        }else{
            $kyc = new UserKyc();
            $kyc->user_id = $id;
            $kyc->pan_file =$in['pan_file'];
            $kyc->adhaar_file =$in['adhaar_file'];
            $kyc->bank_file =$in['bank_file'];
            $kyc->adhaar_no =$request->adhaar_no;
            $kyc->pancard =$request->pancard;
            $kyc->save();
        }
    ...
    public function approvekyc(Request $request)
    {
        ...
        $kyc =  UserKyc::where('id',$request->id)->first();
        $kyc->is_approved = 1;
        $kyc->save();
    ...
    public function adhaarDownload($kyc_id)
    {
        $attachment = UserKyc::findOrFail(decrypt($kyc_id));
        $file = $attachment->adhaar_file;
    ...
    public function panDownload($kyc_id)
    {
        $attachment = UserKyc::findOrFail(decrypt($kyc_id));
        $file = $attachment->pan_file;
    ...
    public function bankfileDownload($kyc_id)
    {
        $attachment = UserKyc::findOrFail(decrypt($kyc_id));
        $file = $attachment->bank_file;
...
public function updatebank()
    {
        ...
        $data['page_title'] = "Update Bank Details";
        return view($this->activeTemplate . 'user.bank_detail', $data);
    }
    ...
    public function submitbank(Request $request)
    {
        ...
        $request->validate([
            'bank_acc_no' => 'required|string|max:250',
            'bank_name' => 'required|string|max:250',
            'bank_ifsc_code' => 'required|string|max:250',
            'bank_holder_name' => 'required|string|max:250',
        ...
        $id =$user->id; 
         ...
         $in['bank_acc_no']=$request->bank_acc_no;
         $in['bank_name']=$request->bank_name;
         $in['bank_ifsc_code']=$request->bank_ifsc_code;
         $in['bank_holder_name']=$request->bank_holder_name;
